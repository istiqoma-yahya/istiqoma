...
- **QF credential split (intentional)**: The Content proxy runs against the **production** QF environment (`QF_CONTENT_ENV=production`, `QF_CONTENT_CLIENT_ID/SECRET`) while the User APIs run against **pre-live** (`QF_ENV=prelive`, `QF_USER_CLIENT_ID/SECRET`). This is by design — at hackathon time only the production client has full content scope, and only the pre-live client has the `bookmark` scope plus a registered callback. Once QF approves user-feature scopes on the production client, both can be unified. The OAuth callback URL is fixed via `QF_REDIRECT_URI=https://www.istiqoma.com/api/qf/callback` (the only callback registered with QF), so the connect flow only completes on the deployed site, not in dev preview.
- **Quran Foundation User API — Bookmarks**: `server/qf-user.ts` implements OAuth2 Authorization Code + PKCE (`scope=openid offline bookmark`). Users opt in from a "Connect Quran Foundation" card on `/profile` (`QuranFoundationConnectCard` in `client/src/pages/ProfilePage.tsx`). Tokens are stored in `qf_user_tokens` (one row per user). When connected, bookmark add/remove is mirrored to QF in the background and `GET /api/quran/bookmarks` merges remote bookmarks into the local list. Mirroring is non-fatal — the local DB is always the source of truth, so a QF outage never breaks the response.
